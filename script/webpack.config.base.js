...
function genNestedEntry(dir) {
  ...
  arr.forEach(filePath => {
    const key = filePath.replace(dir + '/', '').replace('.js', '');
    entry[key] = filePath;
  });
  return entry;
}
...
module.exports = function(options = {}) {
  ...
  const notProduction = env.notProduction();
  ...
  const babelPresets = [['stage-0'], ['env', { modules: babelModules }]];
  if (vue) {
  ...
  let config = {
    ...
    plugins: [
      ...
      new webpack.DefinePlugin({
        'process.env': {
          NODE_ENV: JSON.stringify(env.getNodeEnv()),
